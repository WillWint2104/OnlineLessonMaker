...
const OUT = path.join(root, 'docs/atlas/app-lesson');
fs.mkdirSync(OUT, { recursive: true });
const LESSON = JSON.parse(fs.readFileSync(path.join(root, 'docs/atlas/lesson/quadratics.app.json'), 'utf8'));
...
};
{
  ...
  await p.evaluate(() => document.querySelector('[data-mx-tab="g-symmetry"]').click());
  const sym = GROUPS.find((g) => g.id === 'g-symmetry');
  const states = [];
  for (const st of sym.states) {
    await p.evaluate((sid) => document.querySelector(`[data-mx-panel="g-symmetry"]`).querySelector(`[data-mx-state="${sid}"]`).click(), st.id);
    await p.waitForTimeout(300);
    states.push(await p.evaluate(() => {
      const pane = document.querySelector('[data-mx-panel="g-symmetry"]');
  ...
  await p.evaluate(() => document.querySelector('[data-mx-panel="g-symmetry"]').querySelector('[data-mx-state="visual"]').click());
  await p.waitForTimeout(300);
  ...
  const tbl = GROUPS.flatMap((g) => (g.examples || [])).flatMap((e) => (e.steps || []))
    .map((s) => s.visual).filter((v) => v && (v.kind === 'table' || (v.parts || []).some((q) => q.kind === 'table')));
  const tblSpec = tbl.map((v) => (v.kind === 'table' ? v : v.parts.find((q) => q.kind === 'table')))[0];
