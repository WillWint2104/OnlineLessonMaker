...
- Give note, question and subpart actions distinct accessible row names; handle a null inspector item defensively without changing its index.
- Preserve raw imported companion indexes during nested edits and structural changes, and retain the corpus-reference fallback for local-only checkouts.
